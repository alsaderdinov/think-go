...
import (
	...
	"think-go/hw-11/pkg/crawler/spider"
	"think-go/hw-11/pkg/index"
	"think-go/hw-11/pkg/netsrv"
)
...
func main() {
	...
	if fileExists(file) {
		...
		if err := saveDocs(file, docs); err != nil {
			fmt.Println("Error saving documents:", err)
			return
		}
	}

	indexedDocs := indexDocs(idx, docs)
	server := netsrv.New(host, port, idx, indexedDocs)
